...
#include "yolo_pointcloud_detect/preprocess.h"
...
#include "yolo_pointcloud_detect/draw.h"
...
YoloDetector::YoloDetector(const std::string trtFile): trtFile_(trtFile)
{
    ...
    context = engine->createExecutionContext();
        ...
        // 1. 设置输入 shape
        const char* inputName = "images"; // tensor name
        nvinfer1::Dims inputDims;
        inputDims.nbDims = 4;  // NCHW
        inputDims.d[0] = 1;    // batch
        inputDims.d[1] = 3;    // channels
        inputDims.d[2] = kInputH; // height
        inputDims.d[3] = kInputW; // width

        if (!context->setInputShape(inputName, inputDims)) {
            std::cerr << "Failed to set input dims!" << std::endl;
        }

        // 2. 获取输出 Tensor 名称
        const char* outputName1 = "output1";  // 输出索引 2 proto [1 32 160 160]
        // 3. 获取输出维度（TRT10 新 API）
        protoOutDims = context->getTensorShape(outputName1);
        int protoOutputSize = 1;  // 32 * 160 * 160
        for (int i = 0; i < protoOutDims.nbDims; i++){
            protoOutputSize *= protoOutDims.d[i];
        }
        std::cout<< "Proto output dims: " << shapeToString(protoOutDims) << std::endl;

        const char* outputName2 = "output0";  // 输出索引 3 [1 116 8400], 116 = 4 + 80 + 32 = bbox + class + mask coefficients
        nvinfer1::Dims outDims = context->getTensorShape(outputName2);  // [1 116 8400], 116 = 4 + 80 + 32 = bbox + class + mask coefficients
        OUTPUT_CANDIDATES = outDims.d[2];  // 8400
        std::cout<< "Output dims: " << shapeToString(outDims) << std::endl;
    #else
    ...
    std::cout << "protoOutputSize = " << protoOutputSize <<std::endl;//!!!一定确认好
    ...
    CHECK(cudaMalloc(&vBufferD[1], protoOutputSize * sizeof(float)));//32*160*160 output1
    CHECK(cudaMalloc(&vBufferD[2], outputSize * sizeof(float)));//116*8400 output0
...
std::vector<Detection> YoloDetector::inference(cv::Mat& img){
    ...
    // tensorrt inference
        ...
        //     context->setTensorAddress(name, vBufferD.data()[i]);
        // }
        std::cout << "Binding " << 0 << ": " << engine->getIOTensorName(0) << std::endl;
        auto const name0 = engine->getIOTensorName(0);
        context->setTensorAddress(name0, vBufferD.data()[0]);

        std::cout << "Binding " << 1 << ": " << engine->getIOTensorName(1) << std::endl;
        auto const name1 = engine->getIOTensorName(1);
        context->setTensorAddress(name1, vBufferD.data()[2]);

        std::cout << "Binding " << 2 << ": " << engine->getIOTensorName(2) << std::endl;
        auto const name2 = engine->getIOTensorName(2);
        context->setTensorAddress(name2, vBufferD.data()[1]);
    ...
    transpose((float*)vBufferD[2], transposeDevide, OUTPUT_CANDIDATES, 4 + kNumClass + 32, stream);//!!!!这里要特别注意vBufferD[0] binding images , vBufferD[1] binding output[1], vBufferD[2] binding binding output[0]
    ...
    void YoloDetector::process_mask(
        float* protoDevice, nvinfer1::Dims protoOutDims, std::vector<Detection>& vDetections, 
        ...
        //cudaMemcpy(hostMask, maskDevice, 160*160*sizeof(float), cudaMemcpyDeviceToHost);
