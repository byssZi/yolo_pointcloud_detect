...
public:
  ...
  std::pair<pcl::PointCloud<PointXYZRGBIL>::Ptr, cv::Mat> ProjectToRawMat(cv::Mat outImg, cv::Mat K, cv::Mat D, cv::Mat R,
                          cv::Mat T, std::vector<Detection> segments) {
    ...
    pcl::PointCloud<PointXYZRGBIL>::Ptr fusion_pcl_ptr (new pcl::PointCloud<PointXYZRGBIL>);   //放在这里是因为，每次都需要重新初始化
    if (oriCloud.empty() || T_.empty() || oriCloud.cols == 0 || T_.cols == 0) {
        ...
        return std::pair<pcl::PointCloud<PointXYZRGBIL>::Ptr, cv::Mat>(fusion_pcl_ptr, outImg);
    }
    ...
    for (int32_t i = 0; i < projCloud2d.cols; ++i) {
      ...
      if (x2d >= 0 && y2d >= 0 && x2d < outImg.cols && y2d < outImg.rows && z > 0) {
        ...
        bool has_object_match = false;
        int label = -1;
        for (size_t j = 0; j < segments.size(); ++j) {
          if (segments[j].maskMatrix[y2d * outImg.cols + x2d] > 0.5) {
            has_object_match = true;
            label = segments[j].classId;
            break;
          }
        }
    ...
    if (overlap_filter_) {
      ...
      for (int32_t m = 0; m < outImg.rows; m++) {
        for (int32_t n = 0; n < outImg.cols; n++) {
          ...
          for (int j = std::max(0, m - OVERLAP_FILTER_WINDOW);
               j < std::min(outImg.rows, m + OVERLAP_FILTER_WINDOW + 1); j++) {
            for (int k = std::max(0, n - OVERLAP_FILTER_WINDOW);
                 k < std::min(outImg.cols, n + OVERLAP_FILTER_WINDOW + 1); k++) {
              if (filter_pts[j][k] == -1)
                continue;
